...
# --- 2. SISTEMA DE FAVORITOS (Adaptado para Session State) ---
    ...
    if st.button("📊 Projetar Renda e Rebalancear"):
        with st.spinner('Calculando...'):
            ...
            if lista_dados_rebal:
                ...
                patrimonio_total_novo = df_merged['Valor Atual'].sum() + novo_aporte
                ...
                df_merged['Renda Mensal Média'] = (df_merged['Qtd Final'] * df_merged['Div_Anual']) / 12
                ...
                c3.metric("Renda Anual", f"R$ {(df_merged['Renda Mensal Média'].sum() * 12):,.2f}")
                
                st.write("### 🛒 Ordem de Compra")
                st.dataframe(df_merged[df_merged['Comprar (Qtd)'] > 0][['Ação', 'Preço', 'Comprar (Qtd)', 'Renda Mensal Média']], use_container_width=True)
